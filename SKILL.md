...
| 命令 | 作用 |
|------|------|
| `extract` | 打开阅读页 → 自动提取数字 bookId → 调全书搜索 API `web/book/search?keyword=《`（server-side、零翻页）→ 正则提取《…》去重 → 写 `books.txt` + `weread_mentioned_books.md` |
| `expand` | 扩展阅读：复用 `extract` 搜索 API 提取逻辑 → 生成带微信读书链接的「扩展阅读清单」`weread_reading_list.md`（**不自动加书架**） |
...
针对「某本书里提到的书，想做扩展阅读」的场景：复用 `extract` 的提取逻辑（拦截章节接口回包 + PageDown 翻页遍历全书 → 解码 + 正则去重），但**不自动加书架**，而是生成一份带微信读书链接的「扩展阅读清单」`weread_reading_list.md`（每本书一行，点击即跳到该书在微信读书的检索页，默认中文优先）。同时仍产出 `books.txt`，后续想加书架直接 `shelf --books-file books.txt` 即可，无需重新提取。两者共用 `collect_mentioned()`，逻辑只维护一处。
...
7. **extract 走全书搜索 API，不扫 DOM、不翻页**：微信读书正文经过虚拟化 + 懒加载，正文《书名》不进 DOM；章节接口 `c` 签名绑定、无法离线重放。故改为调**阅读器"查询"功能的后端接口**：`GET /web/book/search?bookId=<数字>&keyword=《`，服务器全量返回结果（含 abstract 上下文片段 + hasMore 分页），正则从 abstract 提取书名即可。这是正常用户搜索，零反爬风险。数字 bookId 从页面 performance 资源 URL 或封面图 `YueWen_` 前缀自动提取。
...
- **`extract` 走搜索 API，headless 完全可用**（v1.2.1）：不需要真实渲染、不需要翻页、纯 HTTP 调用 server-side 搜索。只需登录态即可。
